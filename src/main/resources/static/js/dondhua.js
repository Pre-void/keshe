...
function controllPath(addBtn, shopCar) {
    ...
    var ballTimer = setInterval(function () {
        //每次重新坐标
        xAbscissa += 100 * relativePosition;
        yAbscissa = (yDistance / Math.pow(xDistance, 2)) * Math.pow(xAbscissa, 2);
        ballDom.style.top = addBtnCenterY + yAbscissa - 5 + "px";
        ballDom.style.left = addBtnCenterX + xAbscissa  - 3 + "px";

        //检查是否到达终点
        var surplusDistance = parseInt(ballDom.style.left) - shopCarCenterX;
        if (Math.abs(surplusDistance) <= 20) {
...
function drawBall() {
    ...
    ballDom.style.width = "15px";
    ballDom.style.height = "15px";
    // ballDom.style.border = "1px solid #ccsc";
    ballDom.style.background = "#ff8800";
